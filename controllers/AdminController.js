	...
	constructor(adminService) {
		...
		this.router.get('/sky', async (req, res) => {
			...
			if (ensureAuthorised(req, res)) {
				res.render(path.resolve(process.env.PWD, 'public', 'layout', 'admin_table'), {
		...
		this.router.get('/allsky', async (req, res) => {
			...
			if (ensureAuthorised(req, res)) {
				res.render(path.resolve(process.env.PWD, 'public', 'layout', 'admin_all_table'), {
					grouped
				})
			}
		});
		...
		this.router.get('/login', (req, res) => {
			if (req.session.loggedIn) {
				...
				res.sendFile(path.resolve(process.env.PWD, 'public', 'layout', 'admin_login.html'));
			}
		});
